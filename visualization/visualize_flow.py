...
sequence_loader = ArgoverseSceneFlowSequenceLoader("/efs/argoverse2/val/", use_gt_flow=False)

# sequence_id = sequence_loader.get_sequence_ids()[2]
sequence_id = "0b86f508-5df9-4a46-bc59-5b9536dbde9f"
...
def draw_frames(reset_view=False):
    ...
    for idx, frame_dict in enumerate(tqdm.tqdm(frame_list)):
        ...
        if flowed_pc is not None and idx < len(frame_list) - 1:
            print("Max flow magnitude:", pc.matched_point_distance(flowed_pc).max())
