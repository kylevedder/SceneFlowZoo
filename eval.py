...
def compute_end_point_error(dts: NDArrayFloat,
                            gts: NDArrayFloat) -> NDArrayFloat:
    ...
    """
    end_point_error: NDArrayFloat = np.linalg.norm(dts - gts,
                                                   axis=-1).astype(np.float64)
    return end_point_error


def compute_accuracy(dts: NDArrayFloat, gts: NDArrayFloat,
                     distance_threshold: float) -> NDArrayFloat:
    ...
    relative_error_inlier = np.less(relative_error,
                                    distance_threshold).astype(bool)
    accuracy: NDArrayFloat = np.logical_or(
        abs_error_inlier, relative_error_inlier).astype(np.float64)
    return accuracy


def compute_accuracy_strict(dts: NDArrayFloat,
                            gts: NDArrayFloat) -> NDArrayFloat:
...
def compute_accuracy_relax(dts: NDArrayFloat,
                           gts: NDArrayFloat) -> NDArrayFloat:
...
def compute_angle_error(dts: NDArrayFloat, gts: NDArrayFloat) -> NDArrayFloat:
    ...
    dts_space_time = np.pad(dts, ((0, 0), (0, 1)),
                            constant_values=constants.SWEEP_PAIR_TIME_DELTA)
    gts_space_time = np.pad(gts, ((0, 0), (0, 1)),
                            constant_values=constants.SWEEP_PAIR_TIME_DELTA)

    dts_space_time_norm = np.linalg.norm(dts_space_time,
                                         axis=-1,
                                         keepdims=True)
    gts_space_time_norm = np.linalg.norm(gts_space_time,
                                         axis=-1,
                                         keepdims=True)
    ...
    angle_error: NDArrayFloat = np.arccos(clipped_dot_product).astype(
        np.float64)
    return angle_error
...
def compute_scene_flow_metrics(
        ...
        scene_flow_metric_type: SceneFlowMetricType) -> NDArrayFloat:
...
def compute_segmentation_metrics(
        dts: NDArrayBool, gts: NDArrayBool,
        segmentation_metric_type: SegmentationMetricType) -> int:
...
def compute_metrics(
    ...
    """
    assert gts.shape == pred_flow.shape, f'gts and pred_flow must have same shape, but got {gts.shape} and {pred_flow.shape}'

    point_cloud = point_cloud[is_valid].astype(np.float64)
    pred_flow = pred_flow[is_valid].astype(np.float64)
    pred_dynamic = pred_dynamic[is_valid].astype(bool)

    # print(
    #     f"Num pred dynamic: {pred_dynamic.sum():6d} | Fraction dynamic: {(pred_dynamic.sum() / len(pred_dynamic)*100.0) :05.03f}%"
    # )
    ...
    if idx >= 125 and False:
        print("Annotation file: ", anno_file)
        import open3d as o3d

        def make_lineset(pc, flowed_pc, draw_color, line_mask=None):
            if line_mask is None:
                line_mask = np.ones(len(pc)).astype(bool)
        ...
        pcd.colors = o3d.utility.Vector3dVector(
            np.ones_like(point_cloud) * [0.1, 0.0, 0.0])

        pcd_next = o3d.geometry.PointCloud()
        pcd_next.points = o3d.utility.Vector3dVector(next_point_cloud)
        pcd_next.colors = o3d.utility.Vector3dVector(
            np.ones_like(next_point_cloud) * [0, 0, 1])

        pred_lineset = make_lineset(point_cloud, point_cloud + pred_flow,
                                    [1, 0, 0])

        pcd_gt = o3d.geometry.PointCloud()
        pcd_gt.points = o3d.utility.Vector3dVector(point_cloud + gts)
        pcd_gt.colors = o3d.utility.Vector3dVector(
            np.ones_like(point_cloud) * [0, 1, 0])

        gt_lineset = make_lineset(point_cloud, point_cloud + gts, [0, 1, 0])

        o3d.visualization.draw_geometries([pcd, pcd_next, pred_lineset, gt_lineset])
    ...
    for cls, category_idxs in metric_categories.items():
        ...
        for i in category_idxs[1:]:
            category_mask = np.logical_or(category_mask,
                                          (category_indices == i))

        for motion, m_mask in [("Dynamic", is_dynamic),
                               ("Static", ~is_dynamic)]:
...
def evaluate_predictions(
        ...
        get_prediction: Callable[[Path], pd.DataFrame]) -> pd.DataFrame:
    ...
    for idx, anno_file in enumerate(tqdm.tqdm(annotation_files)):
        ...
        current_example_results = compute_metrics(
            anno_file,
            idx,
            pred[["point_x", "point_y", "point_z"]].to_numpy().astype(float),
            pred[["point_x_next", "point_y_next",
                  "point_z_next"]].to_numpy().astype(float),
    ...
    df = pd.DataFrame(
        ...
        list(SceneFlowMetricType) + list(SegmentationMetricType),
    )
    return df
...
def get_prediction_from_directory(
        ...
        predictions_dir: Path) -> Optional[pd.DataFrame]:
...
def get_prediction_from_zipfile(
        annotation_name: Path,
        predictions_zip: Path) -> Optional[pd.DataFrame]:
...
def evaluate_directories(annotations_dir: Path,
                         predictions_dir: Path) -> pd.DataFrame:
    ...
    return evaluate_predictions(
        annotations_dir,
        lambda n: get_prediction_from_directory(n, predictions_dir))
...
def evaluate_zip(annotations_dir: Path, predictions_zip: Path) -> pd.DataFrame:
    ...
    return evaluate_predictions(
        annotations_dir,
        lambda n: get_prediction_from_zipfile(n, predictions_zip))
...
def results_to_dict(frame: pd.DataFrame) -> Dict[str, float]:
    ...
    def weighted_average(
        x: pd.DataFrame, metric_type: Union[SceneFlowMetricType,
                                            SegmentationMetricType]) -> float:
    ...
    for metric_type in SceneFlowMetricType:
        ...
        for segment in segments:
            ...
            metric_type_str = (metric_type.title().replace("_", " ")
                               if metric_type != SceneFlowMetricType.EPE else
                               metric_type)
    ...
    for metric_type in SceneFlowMetricType:
        avg_nodist: pd.Series[float] = grouped.apply(
            lambda x, m=metric_type: weighted_average(x, metric_type=m))
        segments_nodist: List[Tuple[str, str,
                                    str]] = avg_nodist.index.to_list()
    ...
    output["Dynamic IoU"] = frame.TP.sum() / (frame.TP.sum() + frame.FP.sum() +
                                              frame.FN.sum())
...
def evaluate(annotations_dir: str, predictions_dir: str) -> Dict[str, float]:
    ...
    """
    results_df = evaluate_directories(Path(annotations_dir),
...
def _evaluate_entry(annotations_dir: str,
                    predictions_dir: str) -> Dict[str, float]:
