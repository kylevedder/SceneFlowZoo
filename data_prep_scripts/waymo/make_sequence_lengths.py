...
input_folders = sorted(e for e in sequence_folder.iterdir() if e.is_dir())
output_file = sequence_folder.parent / f"{sequence_folder.stem}_sequence_lengths.json"

print(f"Counting the number of files under {sequence_folder}/* and saving to {output_file}")
...
name_lookup = {folder.stem: process_folder(folder) for folder in input_folders}
