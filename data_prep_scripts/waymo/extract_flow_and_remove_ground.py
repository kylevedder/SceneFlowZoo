...
def get_ground_heights(
    raster_heightmap, global_to_raster_se2, global_to_raster_scale, global_point_cloud: PointCloud
) -> np.ndarray:
    ...
    ind_valid_pts = ~np.logical_or(
        np.logical_or(outside_max_x, outside_max_y), np.logical_or(outside_min_x, outside_min_y)
    )
...
def is_ground_points(
    ...
    ground_height_values = get_ground_heights(
        raster_heightmap, global_to_raster_se2, global_to_raster_scale, global_point_cloud
    )
    ...
    for idx, data in enumerate(dataset):
        ...
        save_pickle(
            save_folder / f"{idx:06d}.pkl",
            {
                ...
                "label": masked_label,
                "pose": pose.to_array(),
                "fraction_kept": np.sum(keep_points_mask) / len(keep_points_mask),
            },
        )
