...
def warped_pc_loss(warped_pc: torch.Tensor, target_pc: torch.Tensor, dist_threshold: float = 2.0):
    if warped_pc.ndim == 2:
    ...
    assert (
        warped_pc.ndim == 3
    ), f"warped_pc.ndim = {warped_pc.ndim}, not 3; shape = {warped_pc.shape}"
    assert (
        target_pc.ndim == 3
    ), f"target_pc.ndim = {target_pc.ndim}, not 3; shape = {target_pc.shape}"

    loss = 0

    if dist_threshold is None:
        loss += chamfer_distance(warped_pc, target_pc, point_reduction="mean")[0].sum()
        loss += chamfer_distance(target_pc, warped_pc, point_reduction="mean")[0].sum()
        return loss
...
def pc0_to_pc1_distance(
    pc0: Union["PointCloud", torch.Tensor], pc1: Union["PointCloud", torch.Tensor]
):
    ...
    pc0_to_pc1_knn = knn_points(
        p1=pc0, p2=pc1, lengths1=pc0_shape_tensor, lengths2=pc1_shape_tensor, K=1
    )
