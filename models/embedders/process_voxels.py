    ...
    def __init__(
        ...
        for i in range(len(feat_channels) - 1):
            ...
            if i < len(feat_channels) - 2:
                ...
                last_layer = True
            pfn_layers.append(PFNLayer(in_filters, out_filters, last_layer=last_layer, mode=mode))
...
class DynamicPillarFeatureNet(PillarFeatureNet):
    ...
    def __init__(
        ...
        self.pfn_layers = nn.ModuleList(pfn_layers)
        self.pfn_scatter = DynamicScatterWrapper(voxel_size, point_cloud_range, (mode != "max"))
        self.cluster_scatter = DynamicScatterWrapper(
    ...
    def map_voxel_center_to_point(self, pts_coors, voxel_mean, voxel_coors):
        ...
        if pts_coors.shape[0] == 0:
            return torch.zeros(
                (0, voxel_mean.shape[1]), dtype=voxel_mean.dtype, device=voxel_mean.device
            )
        # Step 1: scatter voxel into canvas
        # Calculate necessary things for canvas creation
        assert (
            voxel_mean.shape[0] == voxel_coors.shape[0]
        ), f"voxel_mean.shape[0] {voxel_mean.shape[0]} != voxel_coors.shape[0] {voxel_coors.shape[0]}"
        assert pts_coors.shape[1] == 3, f"pts_coors.shape[1] {pts_coors.shape[1]} != 3"
        assert voxel_coors.shape[1] == 3, f"voxel_coors.shape[1] {voxel_coors.shape[1]} != 3"
        ...
        voxel_index = (
            pts_coors[:, 0] * canvas_y * canvas_x + pts_coors[:, 2] * canvas_x + pts_coors[:, 1]
        )
