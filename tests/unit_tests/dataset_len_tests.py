import pytest
from dataloaders import BucketedSceneFlowDataset, BucketedSceneFlowInputSequence
from pathlib import Path
import torch


@pytest.fixture
def first_split_dataset() -> BucketedSceneFlowDataset:
...
@pytest.fixture
def second_split_dataset() -> BucketedSceneFlowDataset:
    dataset = BucketedSceneFlowDataset(
        dataset_name="Argoverse2CausalSceneFlow",
        root_dir=Path("/tmp/argoverse2_small/val/"),
        flow_data_path=Path("/tmp/argoverse2_small/val_sceneflow_feather/"),
...
@pytest.fixture
def third_split_dataset() -> BucketedSceneFlowDataset:
    dataset = BucketedSceneFlowDataset(
        dataset_name="Argoverse2CausalSceneFlow",
        root_dir=Path("/tmp/argoverse2_small/val/"),
        flow_data_path=Path("/tmp/argoverse2_small/val_sceneflow_feather/"),
...
@pytest.fixture
def full_dataset() -> BucketedSceneFlowDataset:
    dataset = BucketedSceneFlowDataset(
        dataset_name="Argoverse2CausalSceneFlow",
        root_dir=Path("/tmp/argoverse2_small/val/"),
        flow_data_path=Path("/tmp/argoverse2_small/val_sceneflow_feather/"),
...
def test_load_first_split_datset(first_split_dataset: BucketedSceneFlowDataset):
    assert (
        len(first_split_dataset) == 104
    ), f"Expected 104 sequences, but got {len(first_split_dataset)}."


def test_load_second_split_datset(second_split_dataset: BucketedSceneFlowDataset):
    assert (
        len(second_split_dataset) == 103
    ), f"Expected 103 sequences, but got {len(second_split_dataset)}."


def test_load_third_split_datset(third_split_dataset: BucketedSceneFlowDataset):
    assert (
        len(third_split_dataset) == 103
    ), f"Expected 103 sequences, but got {len(third_split_dataset)}."


def test_load_full_datset(full_dataset: BucketedSceneFlowDataset):
...
def test_load_full_and_first_split_get_same_elements(
    full_dataset: BucketedSceneFlowDataset, first_split_dataset: BucketedSceneFlowDataset
):
...
def test_load_full_and_second_split_get_same_elements(
    full_dataset: BucketedSceneFlowDataset, second_split_dataset: BucketedSceneFlowDataset
):
...
def test_load_full_and_third_split_get_same_elements(
    full_dataset: BucketedSceneFlowDataset, third_split_dataset: BucketedSceneFlowDataset
):
