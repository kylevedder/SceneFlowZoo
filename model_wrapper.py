...
class ModelWrapper(pl.LightningModule):
    ...
    def validation_step(
        ...
        est_pc2_arrays = [
            pc1_array + est_flow
            for pc1_array, est_flow in zip(pc1_arrays, est_flows)
        ]
    ...
    def _process_val_files(self):
        assert self.scene_trajectory_output_folder is not None, f"scene_trajectory_output_folder is None"
        pickle_files = sorted(
            Path(self.scene_trajectory_output_folder).glob("*.pkl"))
        for pickle_file in tqdm.tqdm(pickle_files):
            ...
            for dataset_idx, query_timestamps, pc_lookup_size, pc1, pc2, est_pc1_flows_valid_idx in zip(
                    dataset_idxes,
