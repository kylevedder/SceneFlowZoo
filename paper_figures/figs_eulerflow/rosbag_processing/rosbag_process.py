    ...
    with multiprocessing.Pool() as pool:
        with rosbag.Bag(str(bag_path), "r") as bag:
            ...
            for last_cloud_msg, last_image_msg in tqdm.tqdm(matched_frames):
                ...
                rgb_images.append(cv_image)

    return colored_point_clouds, rgb_images


def save_point_clouds_as_pickle(
    point_clouds: List[np.ndarray], images: List[np.ndarray], output_folder: Path
) -> None:
    """
    ...
    for i, point_cloud in enumerate(point_clouds):
        ...
        image = images[i]
        output_file = output_folder / f"image_{i:04d}.png"
...
def main() -> None:
    ...
    colored_point_clouds, images = extract_colored_point_clouds(
        args.bag_path, args.cloud_topic, args.image_topic, args.color_camera_info_topic
    )
    save_point_clouds_as_pickle(colored_point_clouds, images, args.output_folder)
