...
def voronoi_landscape(n=1000, w=10, h=5):
    ...
    regions = [
        region for region in regions if np.all([np.linalg.norm(verts[i]) < 1.2 for i in region])
    ]
    ...
    for face in faces:
        ...
        bmesh.ops.scale(
            bm, vec=Vector((0.8, 0.8, 0.8)), verts=f.verts, space=Matrix.Translation(-center)
        )
...
if __name__ == "__main__":
    ...
    # Render scene
    utils.render("validation_results", "point_cloud_render", 512, 512, render_engine="CYCLES")
